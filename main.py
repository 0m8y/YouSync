...
path_to_save_audio2_test = "D:\\OneDrive\\Epsi\\dev_perso\\YouSync\\MyPlaylistTest"
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(download_audio_from_youtube, url, path_to_save_audio2_test) for url in video_urls]
        for future in futures:
            if future.result():
                videos_downloaded.append(future.result())

    for url in videos_downloaded:
        add_metadata_to_mp3(url, path_to_save_audio2_test)
