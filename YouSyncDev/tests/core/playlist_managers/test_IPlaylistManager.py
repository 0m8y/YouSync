...
class DummyPlaylistManager(IPlaylistManager):
    def new_audio_manager(self, url: str) -> IAudioManager:
        return DummyAudioManager(
            ...
            data_filepath=self.data_store.filepath,
            lock=self.lock,
            id="dummy_id",
...
def test_playlist_json_created(temp_path):
    try:
        ...
        assert os.path.exists(manager.data_store.filepath)
        with open(manager.data_store.filepath) as f:
            data = json.load(f)
...
def test_update_path(temp_path):
    ...
    try:
        ...
        assert new_path in manager.data_store.filepath
        assert os.path.exists(manager.data_store.filepath)
    finally:
...
def test_update_adds_and_removes_audio(temp_path):
    try:
        ...
        audio = manager.new_audio_manager("https://video.test/1")
        manager.audio_managers.append(audio)
        manager.playlist_data.audios.append(audio.metadata)
...
def test_add_and_remove_audio(temp_path):
    try:
        ...
        url = "https://video.test/99"
        audio = manager.new_audio_manager(url)
        manager.audio_managers.append(audio)
        manager.playlist_data.audios.append(audio.metadata)
        assert audio in manager.audio_managers
        manager.playlist_data.audios = [a for a in manager.playlist_data.audios if a.url != url]
        manager.audio_managers = [am for am in manager.audio_managers if am.get_url() != url]
